...
= Preliminaries
The most important preliminary concept to address is that the game is a symmetric game. The symmetry here is not spatial in nature (even though the game does have a sort of spatial symmetry to it). Rather, it means that the payoffs for playing any strategy depend only on the the strategies employed by other players. Intuitively, this must be true, as the problem did not define payouts for any specific player. Formally speaking, we define this as follows: for any permutation $pi$, $g_pi(i) (s_1, s_2, ..., s_i, ..., s_N) = g_i (s_pi(1), s_pi(2), ..., s_pi(i), ..., s_pi(N))$. In practice, this also means that whether a given portfolio of strategies is at equilibrium depends only on the number of candidates who choose each strategy. As such, for the purposes of this paper, we will be able to represent any portfolio of strategies with a vector of counts, where the value at position $i$ will represent the number of candidates who chose strategy $i$, which we will denote $sigma in RR^R$. This is a very useful feature of the game, as it drastically reduces our solution space, while allowing us to use some interesting features and variations of the representation to capture important information about the equilibria.
