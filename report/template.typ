...
#let ams-article(
  ...
  // final "and".
  let names = authors.map(author => author.name + " (" + author.cid + ")")
  ...
  show figure.where(kind: "proposition"): it => block(above: 11.5pt, below: 11.5pt, {
    strong({
      it.supplement
      if it.numbering != none {
        [ ]
  ...
  heading(numbering: none)[Acknowledgement of the Use of AI Tools]
  [AI tools (OpenAI's ChatGPT) were used to identify connections to unfamiliar ideas, in particular in helping to identify the link between the game and the Hotellings-Down problem, as well as to generate the idea of using EMD to compare distributions. It was not used for any computations or within the final paper.]
  v(1fr)
